...
class Settings(BaseSettings):
    ...
    # MCP配置
    MCP_SCRIPT_PATH: str = Field(default="../MCP_Client/mcp_client.py", env="MCP_SCRIPT_PATH")
    ...
    def CORS_ORIGINS(self) -> List[str]:
        """解析CORS_ORIGINS字符串为列表"""
        if not self.CORS_ORIGINS_STR:
            return ["http://localhost:3000", "http://localhost:3001"]
        return [origin.strip() for origin in self.CORS_ORIGINS_STR.split(",")]
    
    # JWT配置
    JWT_SECRET: str = Field(default="", env="JWT_SECRET")  # 移除硬编码密钥
    ...
    def __init__(self, **data: Any):
        ...
        # 打印关键配置信息，帮助调试
        logger.info(f"加载配置: ENV={self.ENV}, DEBUG={self.DEBUG}")
        logger.info(f"LLM_MODEL={self.LLM_MODEL}, API_BASE存在={'是' if self.LLM_API_BASE else '否'}")
        logger.info(f"MCP_SCRIPT_PATH={self.MCP_SCRIPT_PATH}")
