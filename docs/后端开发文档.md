...
6. 核心服务组件
   ...
   - 6.4 客户端（Clients）
7. API 接口定义
   - 7.1 认证接口：/api/v1/auth (注册、登录、获取用户信息)
   - 7.2 意图解析：/api/v1/interpret (解析用户意图)
   - 7.3 工具执行：/api/v1/execute (执行工具)、/api/v1/confirm (确认执行)
   - 7.4 工具管理：/api/v1/tools (获取工具列表)
   - 7.5 开发者接口：/api/v1/dev (工具管理、上传、测试、应用管理)
   - 7.6 MCP管理：/api/v1/mcp/status (服务器状态管理)
   - 7.7 健康检查：/health (服务状态检查)
   
   **API基础信息**：基础URL `http://localhost:3000`，认证方式 Bearer Token (JWT)，在线文档 `/docs`。所有接口返回JSON格式，错误时返回 `{"detail": "错误信息"}`。
8. 错误处理与日志
9. 身份认证（JWT）
10. 集成测试与单元测试
11. 本地启动与部署
...
系统已预置不同角色的测试账号，可直接使用：

| 角色 | 用户名 | 密码 | 权限说明 |
|------|--------|------|----------|
| 普通用户 | `testuser_5090` | `8lpcUY2BOt` | 基础AI功能访问 |
| 开发者 | `devuser_5090` | `mryuWTGdMk` | 基础功能 + 开发者Portal |
| 管理员 | `adminuser_5090` | `SAKMRtxCjT` | 所有功能权限 |
