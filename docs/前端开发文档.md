...
为了简化前后端交互并保证一致性，我们提供了统一的API客户端，封装了所有与后端的通信逻辑：
   ...
   const handleChatRequest = async (message) => {
     try {
       const response = await apiClient.sendChatMessage(
         message,             // 用户消息
         'user123',           // 用户ID
         'session-12345',     // 会话ID (可选)
         { context: 'data' }  // 上下文数据 (可选)
       );
...
3. **发送语音请求**
   ```javascript
   // 处理语音请求
   const handleVoiceRequest = async (voiceText) => {
     try {
...
4. **获取可用服务列表**
   ...
   const fetchServices = async () => {
     try {
       ...
       // 设置服务列表
       setServices(response.data.servers);
     } catch (error) {
...
5. **直接执行MCP服务**
   ```javascript
   // 执行MCP服务
   const executeMcpService = async (serverId, query) => {
     try {
       const response = await apiClient.executeMcpService(
         serverId,            // MCP服务ID
         query,               // 用户查询
         { param: 'value' }   // 附加选项 (可选)
       );
...
API客户端统一了响应格式，无论请求成功或失败，都会遵循统一的结构：

**成功响应：**
```javascript
{
  status: 'success',        // 状态: success, error, waiting
  data: { ... },            // 响应数据
  message: '操作成功',       // 提示消息
  timestamp: '2023-...'     // 时间戳
}
...
try {
  ...
  console.error(error.statusCode);  // HTTP状态码 (如果有)
  console.error(error.data);        // 服务器返回的错误数据 (如果有)
}
```
